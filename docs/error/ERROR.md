...
let value: string = get("UNDEFINED_VAR")
```
...
let alice: Person = Person { name: "Alice" }
...
```xenith
try {
    let response: string = http::get("https://api.example.com")
...
Error XEN006: Invalid JSON
  ...
  |
4 | let data: json = parse(response)
  |                     ^^^^^^^^^^^^^^ invalid JSON format
  |
...
Caused by:
  → Error XEN200: HTTP request failed
    ...
    |
  3 | let response: string = http::get("https://api.example.com")
    |                          ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ connection timeout
```
...
```xenith
try {
    let content: string = read("config.json")
    let config: json = parse(content)
} catch err {
    echo("Failed to load config: {err}")
    # Use default configuration
    let config: json = parse({ "debug": false })
}
```
